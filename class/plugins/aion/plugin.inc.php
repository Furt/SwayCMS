...
class plugin {
	...
	public function register($user, $pass, $email, $cap) {
		...
		$checka = "SELECT `id` FROM account_data WHERE name= '$user'";
		...
		if (empty($this->send_error)) {
			$query = mysql_query("INSERT INTO account_data (name,password) VALUES ('$user','$password')");
			if(!$query) {
	...
	public function login($user, $pass) {
		...
		$ucheck = sprintf("SELECT id FROM `account_data` WHERE name='%s'", addcslashes(mysql_real_escape_string($user),'%_'));
		...
		$pcheck = sprintf("SELECT password FROM `account_data` WHERE name='%s'", addcslashes(mysql_real_escape_string($password),'%_'));
		...
		if (empty($this->send_error)) {
			...
			$query = mysql_query("SELECT id,name,access_level,membership FROM `account_data` WHERE name = '". $user ."'");
			if(!$query) {
